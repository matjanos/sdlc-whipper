...
M1+M2 skeleton plus M3 plumbing hardening: ports, contracts, PR idempotency, GitHub checks, and Vercel preview observation are real and tested; **prompts are stub-grade by design** until M4. The M2 spike is closed: event/message/catalog shapes are verified against a live server and pinned by fixtures (`test/runtime-payloads.spec.ts`, `test/runtime-usage.spec.ts`); the env-gated live smoke (`SDL_LIVE_SMOKE=1`, `test/runtime-live.spec.ts`) re-checks preflight, extraction, and the ledger feed in seconds. `costUsd` comes from server-reported usage events. A real deployed target-repo PR is still required to close M3.
